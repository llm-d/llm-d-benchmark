...
Everything else - `kubectl`, `helm`, `helmfile`, `kind`, `helm-diff` - will be installed for you by `./install.sh` in [step 3](#3-install-llmdbenchmark), with one exception: `kind` itself, which we install first below because we want the cluster up before the installer runs.
