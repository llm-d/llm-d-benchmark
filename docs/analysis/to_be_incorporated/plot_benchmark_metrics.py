...
def process_csv_files(benchmark_type, method, benchmark_dir):
    ...
    # Store results
    results = {"qps": [], "itl": [], "ttft": [], "throughput": []}
...
def plot_metrics(results_dict, benchmark_type, title, benchmark_dir, model_name):
    ...
    output_file = os.path.join(
        os.path.dirname(__file__), f"benchmark_metrics_{benchmark_type}.png"
    )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Plot benchmark metrics from CSV files"
    )
    parser.add_argument(
        "--benchmark-dir",
        default=os.path.join(
            os.path.dirname(__file__), "..", "data", "k8s", "lmbenchmark"
        ),
    ...
    for benchmark_type, title in BENCHMARK_TYPES.items():
        ...
        plot_metrics(
            results_dict, benchmark_type, title, args.benchmark_dir, args.model_name
        )
