...
RUN_COLUMNS = {
    ...
    "Directory_Base": ColumnProperties(dtype="str", label="Base Directory"),
    "Start": ColumnProperties(dtype="float", label="Start Time"),
...
def get_benchmark_report_files(
    source_dir: str, recurse_symlinks: bool = False
) -> list[str]:
...
def _get_replicas_and_parallelism(report: BenchmarkReport) -> dict[str, int | None]:
...
def add_benchmark_report_to_df(runs_df: pd.DataFrame, br_file: str) -> None:
    ...
    if (
        report.scenario.platform
        and report.scenario.platform.metadata
        and isinstance(report.scenario.platform.metadata, dict)
    ):
        for plugin in get_nested(
            report.scenario.platform.metadata, ["inferenceScheduler", "plugins"], []
        ):
    ...
    ttft_mult = (
        1000 if report.metrics.latency.time_to_first_token.units == Units.S else 1
    )
    tpot_mult = (
        1000
        if report.metrics.latency.time_per_output_token.units == Units.S_PER_TOKEN
        else 1
    )
    itl_mult = (
        1000
        if report.metrics.latency.inter_token_latency.units == Units.S_PER_TOKEN
        else 1
    )
    ...
    thpt_per_user = div(
        1,
        (div(mul(report.metrics.latency.time_per_output_token.mean, tpot_mult), 1000)),
    )
    ...
    runs_df.loc[len(runs_df)] = {
        ...
        "StdDev_TTFT_ms": mul(
            report.metrics.latency.time_to_first_token.stddev, ttft_mult
        ),
        ...
        "P99.9_TTFT_ms": mul(
            report.metrics.latency.time_to_first_token.p99p9, ttft_mult
        ),
        "Max_TTFT_ms": mul(report.metrics.latency.time_to_first_token.max, ttft_mult),
        # TPOT
        "Mean_TPOT_ms": mul(
            report.metrics.latency.time_per_output_token.mean, tpot_mult
        ),
        "StdDev_TPOT_ms": mul(
            report.metrics.latency.time_per_output_token.stddev, tpot_mult
        ),
        "Min_TPOT_ms": mul(report.metrics.latency.time_per_output_token.min, tpot_mult),
        "P0.1_TPOT_ms": mul(
            report.metrics.latency.time_per_output_token.p0p1, tpot_mult
        ),
        ...
        "P99.9_TPOT_ms": mul(
            report.metrics.latency.time_per_output_token.p99p9, tpot_mult
        ),
        ...
        "StdDev_ITL_ms": mul(
            report.metrics.latency.inter_token_latency.stddev, itl_mult
        ),
...
def get_scenarios(
    runs_df: pd.DataFrame, scenario_columns: list[str], bounded: bool = False
) -> list[dict[str, Any]]:
    ...
    if bounded:
        if not cols_nn:
            raise Exception("Scenario must include at least one non-numeric column")
...
def get_scenario_df(runs_df: pd.DataFrame, scenario: dict[str, Any]) -> pd.DataFrame:
...
def set_scenario_bounds(
    scenario: dict[str, Any], bounds: dict[str, dict[str, int | float]]
) -> dict[str, Any]:
...
def rebound_scenario(runs_df: pd.DataFrame, scenario: dict[str, Any]) -> dict[str, Any]:
...
def print_scenarios(
    ...
    if runs_df is None:
        ...
        header = (
            f"""{Text.BOLD}{Text.BLUE}IDX  {Text.RED}Count  {Text.DEFAULT}{Text.BOLD}"""
        )
...
def make_scenarios_summary_df(
    scenarios: list[dict[str, Any]], runs_df: pd.DataFrame, min_count: int = 0
) -> pd.DataFrame:
    """
    ...
    def col_name(col: str) -> str:
        if col[:BOUND_PREFIX_LEN] in COLUMN_BOUND_STR:
            return col[BOUND_PREFIX_LEN:] + COLUMN_BOUND_STR[col[:BOUND_PREFIX_LEN]]
        return col
    ...
    if scenarios:
        ...
        for col in scenarios[0].keys():
            schema[col_name(col)] = pd.Series(dtype=COLUMNS[col_base(col)].dtype)
...
def get_meet_slo_df(runs_df: pd.DataFrame, slos: list[SLO]) -> pd.DataFrame:
    ...
    for slo in slos:
        if COLUMNS[slo.col].pref == Pref.LOW:
            ...
            runs_meet_slo_df = runs_meet_slo_df[
                runs_meet_slo_df[slo.col].__le__(slo.value)
            ]
        elif COLUMNS[slo.col].pref == Pref.HIGH:
            # Must be greater than or equal to SLO value to meet SLO
            runs_meet_slo_df = runs_meet_slo_df[
                runs_meet_slo_df[slo.col].__ge__(slo.value)
            ]
        else:
...
def get_pareto_front_df(
    runs_df: pd.DataFrame, col_a: str, col_b: str, sort: bool = False
) -> pd.DataFrame:
