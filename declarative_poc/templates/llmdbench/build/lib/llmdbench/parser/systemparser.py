...
class SystemParser:
    ...
    def _get_nested(self, data, path: str):
        ...
        """
        # Normalize bracket indices: "scenario[0]" -> "scenario.0"
        ...
        for part in parts:
            if isinstance(current, dict):
                ...
                # list index lookup
                if not part.isdigit():
                    return {}  # trying to index a list with non-int
                idx = int(part)
                if idx < 0 or idx >= len(current):
                    return {}
                current = current[idx]
            else:
                # neither list nor dict - cannot go deeper
                return {}
    ...
    def _render_template_attribute(self, key):
        ...
        # Merge all scenarios (scenario[0], scenario[1], ...)
