...
def main():
    ...
    for model in model_list:
        ...
        if (
            ev.get("vllm_modelservice_uri_protocol") == "pvc"
            or ev.get("control_environment_type_standalone_active", "0") == "1"
        ):
...
"""
        ...
        helmfile_cmd = (
            f"helmfile --namespace {namespace} "
        ...
        if ev.get("vllm_modelservice_route") == "true" and ev.get("control_deploy_is_openshift", "0") == "1":
            # Check if route exists
            route_name = f"{release}-inference-gateway-route"
            check_route_cmd = (
                f'kubectl --namespace {namespace} get route -o name --ignore-not-found | grep -E "/{route_name}$"'
            )
            ...
            if result != 0:  # Route doesn't exist
                ...
                expose_cmd = (
                    f"kubectl --namespace {namespace} expose service/infra-{release}-inference-gateway "
