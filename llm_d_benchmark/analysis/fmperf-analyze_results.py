...
def analyze_throughput(df, plots_dir):
    ...
    throughput_data = (
        df.groupby("qps")
        .agg({"prompt_tokens": "mean", "generation_tokens": "mean", "generation_time": "mean"})
        .reset_index()
    )
...
def print_statistics(df, data_dir):
    ...
    token_stats = df.agg(
        {"prompt_tokens": ["mean", "std", "min", "max"], "generation_tokens": ["mean", "std", "min", "max"]}
    ).round(4)
...
def main():
    ...
    if "LLMDBENCH_RUN_EXPERIMENT_ANALYZE_LOCALLY" in env_vars and "LLMDBENCH_RUN_EXPERIMENT_LAUNCHER" in env_vars:
        if (
            env_vars["LLMDBENCH_RUN_EXPERIMENT_ANALYZE_LOCALLY"] == "1"
            and env_vars["LLMDBENCH_RUN_EXPERIMENT_LAUNCHER"] == "1"
        ):
            logger.info(
    ...
    parser.add_argument(
        "--results-dir", default=default_dir, help=f"Directory containing the CSV files (default: {default_dir}"
    )
