        ...
        br_dict,
        {
            ...
            "metrics": {
                ...
                "throughput": {
                    ...
                    "total_tokens_per_sec": (results["prompt_tokens"].sum() + results["generation_tokens"].sum())
                    / duration,
...
def import_inference_perf(results_file: str) -> BenchmarkReport:
    ...
    update_dict(
        br_dict,
        {
            ...
            "metrics": {
                "time": {
                    "duration": results["load_summary"][
                        "send_duration"
                    ],  # TODO this isn't exactly what we need, we may need to pull apart per_request_lifecycle_metrics.json
                },
                "requests": {
...
def import_nop(results_file: str) -> BenchmarkReport:
    ...
    results_dict = {
        "scenario": {
            ...
            "platform": {"engine": [results["scenario"]["platform"]["engine"]]},
            "metadata": {
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Convert benchmark run data to standard benchmark report format.")
    parser.add_argument("results_file", type=str, help="Results file to convert.")
    parser.add_argument("output_file", type=str, default=None, nargs="?", help="Output file for benchark report.")
    parser.add_argument(
        "-f", "--force", action=argparse.BooleanOptionalAction, help="Write to output file even if it already exists."
    )
    parser.add_argument(
        "-w",
        "--workload-generator",
