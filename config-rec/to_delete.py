...
# # def inputs():
#     # """
#     # Inputs to the Capacity Planner
#     # """

#     # user_model = st.session_state['model']

#     # # Model
#     # with st.container(border=True):
#     #     st.write("**Model Specification**")
#     #     selected_model = st.text_input("Model (Hugging Face format)", placeholder="ibm-granite/granite-3.3-8b-instruct")

#     #     if selected_model and selected_model != "":

#     #         info = get_model_info_from_hf(selected_model)
#     #         if info is None:
...
#     #     selected_gpu = col1.selectbox("Accelerator", options=st.session_state['gpu_spec'].keys())
#     #     if selected_gpu:
#     #         user_model.gpu_spec = st.session_state['gpu_spec'][selected_gpu]
...
#     #     st.success(f"Loading this model on the selected GPU in {user_model.precision} mode requires a minimum of {min_gpu_req}, which does not yet account for KV cache.")
