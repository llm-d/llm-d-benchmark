...
There are two lines of the benchmark report, `0.1` and `0.2`. All reports are generated by the `llm-d-benchmark` harness pod, but new applications consuming benchmark data should use the `0.2` line, whose current revision is `0.2.1`. Reports that declare `0.2` were produced before that revision and still load: every valid `0.2` report is also a valid `0.2.1` report.
...
The `version` field (`0.2.1` for reports produced by this package, `0.2` for reports produced before that revision) indicates the schema revision for the benchmark report. Different versions may add or drop certain details, so there is no guarantee of "lossless" conversion between benchmark report versions.
...
Revision `0.2.1` adds optional multi-modal payload statistics for image, video, and audio workloads, plus optional treatment-grouping metadata on `scenario.load.metadata` (`treatment`, `treatment_group`, `concurrent_with`). `concurrent_with` names the treatments that ran at the same time against the same endpoint: when it is non-empty the metrics reflect contention from those workloads rather than this one in isolation. Every field introduced is optional, so any `0.2` report validates unchanged under `0.2.1` (enforced by `tests/test_benchmark_report_v0_2_legacy.py`).

The multi-modal additions, all derived from what the client can determine from the payloads it sent, are:
...
* `-b, --br-version`: Target benchmark report version (defaults to `0.1`; `0.2` and `0.2.1` both produce the current `0.2.x` revision, `0.2.1`).
