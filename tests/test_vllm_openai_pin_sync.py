...
def test_modelservice_pin_stays_in_sync_with_defaults_and_doc():
    defaults = yaml.safe_load(DEFAULTS_PATH.read_text(encoding="utf-8"))

    modelservice_pin = defaults["_anchors"]["modelservice_chart_version"]

    assert defaults["chartVersions"]["llmDModelservice"] == modelservice_pin
    assert _doc_pin_for("llmDModelservice") == modelservice_pin
