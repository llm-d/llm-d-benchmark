...
if [[ $LLMDBENCH_ENVIRONMENT_TYPE_STANDALONE_ACTIVE -eq 1 ]]; then
  for model in ${LLMDBENCH_MODEL_LIST//,/ }; do
    ...
    then
      ...
      llmdbench_execute_cmd "${LLMDBENCH_KCMD} run test${model} -n ${LLMDBENCH_OPENSHIFT_NAMESPACE} --attach --restart=Never --rm --image=ubi9/ubi --quiet --command -- bash -c \"curl --no-progress-meter http://${clusterip}:80/v1/models\" | jq ." ${LLMDBENCH_DRY_RUN} ${LLMDBENCH_VERBOSE}
