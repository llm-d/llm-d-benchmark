...
if [[ $LLMDBENCH_USER_IS_ADMIN -eq 1 ]]; then
  ...
  if [[ $(${LLMDBENCH_KCMD} get pods -n kgateway-system --no-headers --ignore-not-found  --field-selector status.phase=Running | wc -l) -ne 0 ]]; then
    ...
    if [[ ! -d gateway-api-inference-extension ]]; then
        llmdbench_execute_cmd "git clone https://github.com/neuralmagic/gateway-api-inference-extension.git" ${LLMDBENCH_DRY_RUN} ${LLMDBENCH_VERBOSE}
    fi
    pushd gateway-api-inference-extension &>/dev/null
    llmdbench_execute_cmd "INFRASTRUCTURE_OVERRIDE=true make environment.dev.kubernetes.infrastructure" ${LLMDBENCH_DRY_RUN} ${LLMDBENCH_VERBOSE}
    popd &>/dev/null
    popd &>/dev/null
  fi
  ...
  if [[ -z ${_wiev1} ]]; then
    ...
    llmdbench_execute_cmd "${LLMDBENCH_KCMD} apply -f https://raw.githubusercontent.com/istio/istio/refs/tags/1.23.1/manifests/charts/base/crds/crd-all.gen.yaml" ${LLMDBENCH_DRY_RUN} ${LLMDBENCH_VERBOSE}
  else
