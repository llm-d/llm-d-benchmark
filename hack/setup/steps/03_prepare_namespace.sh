...
then
  ...
  for model in ${LLMDBENCH_DEPLOY_MODEL_LIST//,/ }; do
    should_create=0
    is_secret=$(${LLMDBENCH_CONTROL_KCMD} --namespace ${LLMDBENCH_OPENSHIFT_NAMESPACE} get secret ${LLMDBENCH_VLLM_COMMON_HF_TOKEN_NAME} --ignore-not-found=true)
    if [[ -z ${is_secret} ]]; then
      should_create=1
    fi

    is_key=$(${LLMDBENCH_CONTROL_KCMD} --namespace ${LLMDBENCH_OPENSHIFT_NAMESPACE} get secret ${LLMDBENCH_VLLM_COMMON_HF_TOKEN_NAME} -o json | jq -r .data | grep token_${LLMDBENCH_MODEL2PARAM[${model}:label]}-instruct || true)
    if [[ -z $is_key ]]; then
      should_create=1
    fi
...
EOF
  ...
  is_qs=$(${LLMDBENCH_CONTROL_KCMD} -n $LLMDBENCH_OPENSHIFT_NAMESPACE get secrets/${LLMDBENCH_VLLM_COMMON_PULL_SECRET_NAME} -o name --ignore-not-found=true | cut -d '/' -f 2)
  if [[ -z $is_qs ]]; then
