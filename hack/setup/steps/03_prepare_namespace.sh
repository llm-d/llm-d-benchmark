...
then
  ...
  cat << EOF > $LLMDBENCH_WORK_DIR/yamls/${LLMDBENCH_CURRENT_STEP}_secret.yaml
apiVersion: v1
kind: Secret
metadata:
...
EOF
  ...
  llmdbench_execute_cmd "${LLMDBENCH_KCMD} apply -f $LLMDBENCH_WORK_DIR/yamls/${LLMDBENCH_CURRENT_STEP}_secret.yaml" ${LLMDBENCH_DRY_RUN} ${LLMDBENCH_VERBOSE}
