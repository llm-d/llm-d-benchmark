...
if [[ ! -d fmperf ]]; then
  llmdbench_execute_cmd "git clone \"${LLMDBENCH_FMPERF_GIT_REPO}\" -b \"${LLMDBENCH_FMPERF_GIT_BRANCH}\"" ${LLMDBENCH_CONTROL_DRY_RUN} ${LLMDBENCH_VERBOSE}
else
  pushd fmperf &>/dev/null
  llmdbench_execute_cmd "git checkout ${LLMDBENCH_FMPERF_GIT_BRANCH}" ${LLMDBENCH_CONTROL_DRY_RUN} ${LLMDBENCH_VERBOSE}
  llmdbench_execute_cmd "git pull" ${LLMDBENCH_CONTROL_DRY_RUN} ${LLMDBENCH_VERBOSE}
  popd &>/dev/null
fi
pushd fmperf &>/dev/null
llmdbench_execute_cmd "git checkout ${LLMDBENCH_FMPERF_GIT_BRANCH}" ${LLMDBENCH_CONTROL_DRY_RUN} ${LLMDBENCH_VERBOSE}
