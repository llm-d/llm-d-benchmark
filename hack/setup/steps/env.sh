...
export LLMDBENCH_KVCM_GIT_BRANCH=${LLMDBENCH_KVCM_GIT_BRANCH:-22c07c8d4593c6597ef4a818c51c99b02be4f6ba}
...
for req in $LLMDBENCH_SCMD $LLMDBENCH_PCMD $LLMDBENCH_KCMD $LLMDBENCH_HCMD kustomize; do
...
export LLMDBENCH_TEMPDIR=${LLMDBENCH_TEMPDIR:-$(mktemp -d -t ${LLMDBENCH_OPENSHIFT_CLUSTER_NAME}-$(echo $0 | rev | cut -d '/' -f 1 | rev | $LLMDBENCH_SCMD -e 's^.sh^^g' -e 's^./^^g')XXX)}
