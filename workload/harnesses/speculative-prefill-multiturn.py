...
``x-speculative-prefill: true`` only on the long-answer turn. The per-turn time
to first streamed token (TTFT) is summarized for both arms.
...
def build_user_plan(args: argparse.Namespace, user_id: int) -> list[dict[str, Any]]:
    ...
    for turn in range(args.num_turns):
        ...
        if turn < args.num_turns - 1 and args.mean_delay_ms > 0:
            mean_delay_seconds = args.mean_delay_ms / 1000.0
            if args.delay_mode == "fixed":
                delay_after_seconds = mean_delay_seconds
            else:
                delay_after_seconds = rng.expovariate(1.0 / mean_delay_seconds)

        plan.append({"turn": turn, "prompt": prompt, "delay_after_seconds": delay_after_seconds})

    return plan


def build_user_plans(args: argparse.Namespace) -> dict[int, list[dict[str, Any]]]:
    return {user_id: build_user_plan(args, user_id) for user_id in range(args.num_users)}
...
def post_stream(
    ...
    request = urllib.request.Request(
        ...
        headers={
            ...
            "X-Benchmark-Case": case_label,
            "X-Benchmark-User-ID": str(user_id),
            "X-Benchmark-Turn": str(turn),
        },
    )
    ...
    return ttft_ms, "".join(answer), finish_reason, start, end
...
def run_user(
    ...
    for entry in user_plan:
        ...
        use_header = use_speculative_prefill and turn == LONG_ANSWER_TURN
        ttft_ms, answer, finish_reason, request_start, request_end = post_stream(
            args.url,
            args.model,
            messages,
            ...
            results.append(
                {
                    ...
                    "turn": turn,
                    "ttft_ms": ttft_ms,
                    "request_start_unix": request_start,
                    "request_end_unix": request_end,
        ...
        if entry["delay_after_seconds"] > 0:
            time.sleep(entry["delay_after_seconds"])
...
def run_case(
    args: argparse.Namespace,
    user_plans: dict[int, list[dict[str, Any]]],
    use_speculative_prefill: bool,
    label: str,
) -> dict[str, Any]:
    print(
        f"\n===== {label} (users={args.num_users} turns={args.num_turns} "
        f"delay={args.mean_delay_ms}ms delay_mode={args.delay_mode} "
        f"header_on_long_turn={use_speculative_prefill}) =====",
        flush=True,
    )
    ...
    with ThreadPoolExecutor(max_workers=args.num_users) as executor:
        futures = [
            executor.submit(run_user, args, user_id, use_speculative_prefill, user_plans[user_id], label)
    ...
    follow_up_ttft = [row["ttft_ms"] for row in rows if row["turn"] > LONG_ANSWER_TURN]
    if follow_up_ttft:
        print(
...
def compare_cases(baseline: dict[str, Any], treatment: dict[str, Any]) -> dict[str, Any]:
    ...
    baseline_follow_up = [row["ttft_ms"] for row in baseline["rows"] if row["turn"] > LONG_ANSWER_TURN]
    treatment_follow_up = [row["ttft_ms"] for row in treatment["rows"] if row["turn"] > LONG_ANSWER_TURN]
    combined = {}
...
def main() -> None:
    args = parse_args()
    user_plans = build_user_plans(args)
    baseline = run_case(args, user_plans, False, "A_baseline")
    treatment = run_case(args, user_plans, True, "B_specprefill")
