"""Step 02 -- Prepare the harness namespace (namespace, HF secret, ConfigMap; PVC + data-access pod in PVC mode)."""
...
class HarnessNamespaceStep(Step):
    ...
    def should_skip(self, context: ExecutionContext) -> bool:
        """Run in every k8s run -- including --no-pvc, which still needs the
        namespace, HF secret, and preprocess ConfigMap (only the PVC and
        data-access pod are skipped, inside execute). Runs regardless of
        deploy method: kustomize guides own their model-side infra, but the
        benchmark harness's infra was never part of the guide. Skip for
        nok8s (no cluster) and collect-only mode (nothing new deployed)."""
    ...
    def _create_hf_token_secret(
        ...
        if not model_ns:
            context.logger.log_warning(
                "No model namespace configured -- cannot copy the HF token "
                "secret into the harness namespace. Create it manually if "
                "harness pods need tokenizer downloads."
            )
            return
        get_result = cmd.kube(
            "get",
