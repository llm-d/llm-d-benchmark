"""
render.py -- Renderer module for llmdbenchmark specification validation and rendering.

This module provides the Renderer class which handles validation of specification
...
class Renderer:
    ...
    def _validate_all_paths(
        ...
        for path, validator, error_msg in validations:
            Renderer._validate_path(path, validator, error_msg, logger)
    ...
    def render(base_dir: Path, specification_file: Path):
        ...
        """
        logger = Renderer._get_logger()

        try:
            templates_dir, scenarios_dir = Renderer._validate_all_paths(
