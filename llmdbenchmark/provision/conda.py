...
from llmdbenchmark.utilities.os.platform import get_platform_dict
...
def install_miniforge_macos(dry_run: bool, verbose: bool):
    ...
    if result.returncode != 0:
        raise RuntimeError(
            f"Failed to install miniforge: {result.stderr if not verbose else ''}"
        )
...
def install_miniforge_linux(dry_run: bool, verbose: bool):
    ...
    platform_info = get_platform_dict()
    ...
    try:
        ...
        if process.returncode != 0:
            error_msg = (
                stderr.decode() if stderr else "Unknown error during installation"
            )
...
def source_conda_script(conda_sh: Path, dry_run: bool, verbose: bool):
    ...
    if result.returncode != 0:
        raise RuntimeError(
            f"Failed to source conda.sh: {result.stderr if not verbose else ''}"
        )
...
def create_conda_environment(env_name: str, dry_run: bool, verbose: bool):
    ...
    if check_conda_environment(env_name):
        announce(
            f'⏭️  Conda environment "{env_name}" already created, skipping installation'
        )
        return 0
    ...
    try:
        ...
        if result.returncode != 0:
            raise RuntimeError(
                f"Failed to create conda environment: {result.stderr if not verbose else ''}"
            )
        ...
        if result.returncode != 0:
            announce(
                f"Warning: conda activate returned {result.returncode} (this is often normal)"
            )

        # Install requirements if available
        requirements_file = (
            Path(os.getenv("LLMDBENCH_MAIN_DIR", ".")) / "build" / "requirements.txt"
        )
    ...
    try:
        ...
        if not is_conda_available():
            ...
            if platform_info["is_mac"]:
                exit_code, anaconda_path, conda_sh = install_miniforge_macos(
                    dry_run, verbose
                )
            elif platform_info["is_linux"]:
                exit_code, anaconda_path, conda_sh = install_miniforge_linux(
                    dry_run, verbose
                )
            else:
